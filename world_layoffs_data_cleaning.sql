...
-- Find duplicate rows in layoffs_staging based on key fields by using ROW_NUMBER()
-- Only returns rows that are duplicates (row_num > 1)
...
/*
 * Create a copy of layoffs_staging with a row_num column added to store row numbers.
 * Use Workbench to copy the CREATE TABLE statement from layoffs_staging and add the row_num field.
 */
...
-- Insert data into layoffs_staging2 with row numbers added to identify duplicates
...
-- Update missing industry values in layoffs_staging2 using a self-join on company
...
/* 
* Fix issue where previous query didn't update everything by setting blank values as NULL
